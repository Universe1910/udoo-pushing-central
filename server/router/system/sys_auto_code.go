...
func (s *AutoCodeRouter) InitAutoCodeRouter(Router *gin.RouterGroup) {
	...
	autoCodeApi := v1.ApiGroupApp.SystemApiGroup.AutoCodeApi
	{
		...
		autoCodeRouter.POST("preview", autoCodeApi.PreviewTemp)         // 获取自动Create 代码预览
		autoCodeRouter.POST("createTemp", autoCodeApi.CreateTemp)       // Create 自动化代码
		autoCodeRouter.POST("createPackage", autoCodeApi.CreatePackage) // Create package包
		autoCodeRouter.POST("getPackage", autoCodeApi.GetPackage)       // 获取package包
		autoCodeRouter.POST("delPackage", autoCodeApi.DelPackage)       // Delete package包

...
func (i *initMenuAuthority) InitializeData(ctx context.Context) (next context.Context, err error) {
	...
	if !ok {
		return ctx, errors.Wrap(system.ErrMissingDependentContext, "Create  [菜单-权限] 关联失败, 未找到权限表初始化数据")
	}
	menus, ok := ctx.Value(initMenu{}.InitializerName()).([]sysModel.SysBaseMenu)
	if !ok {
		return next, errors.Wrap(errors.New(""), "Create  [菜单-权限] 关联失败, 未找到菜单表初始化数据")
	}
	next = ctx
	// 888

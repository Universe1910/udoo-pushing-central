...
func (*AliyunOSS) DeleteFile(key string) error {
	...
	// Delete 单个文件。objectName表示Delete OSS文件时需要指定包含文件后缀在内的完整路径，例如abc/efg/123.jpg。
	// 如需Delete 文件夹，请将objectName设置为对应的文件夹名称。如果文件夹非空，则需要将文件夹下的所有objectDelete 后才能Delete 该文件夹。

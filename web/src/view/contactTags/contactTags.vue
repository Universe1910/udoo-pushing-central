...
import {
  createContactTags,
  deleteContactTags,
  deleteContactTagsByIds,
  updateContactTags,
  findContactTags,
  getContactTagsList
} from '@/api/contactTags'
...
const rule = reactive({
               ...
               tagID : [{
                   required: true,
                   message: 'Tags ID can not be empty',
                   trigger: ['input','blur'],
               }],
})
...
const onDelete = async() => {
      ...
      const res = await deleteContactTagsByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
...
// 更新行
const updateContactTagsFunc = async(row) => {
    const res = await findContactTags({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.recontactTags
...
// Delete 行
const deleteContactTagsFunc = async (row) => {
    const res = await deleteContactTags({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: 'successfully deleted'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createContactTags(formData.value)
                  break
                case 'update':
                  res = await updateContactTags(formData.value)
                  break
                default:
                  res = await createContactTags(formData.value)
                  break

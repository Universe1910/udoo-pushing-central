  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="Action">
            ...
            <el-button type="primary" link icon="edit" size="small" class="table-button" @click="updateTriggerFunc(scope.row)">Edit</el-button>
...
const getTableData = async() => {
  const table = await getTriggerList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteTriggerByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
...
// 更新行
const updateTriggerFunc = async(row) => {
    const res = await findTrigger({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// Delete 行
const deleteTriggerFunc = async (row) => {
    const res = await deleteTrigger({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: 'successfully deleted'

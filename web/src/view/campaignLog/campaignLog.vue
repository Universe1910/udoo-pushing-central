<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="Action" prop="action" width="120" />
        <el-table-column align="left" label="Contact" prop="contactID" width="120" />
        <el-table-column align="left" label="Message" prop="message" width="120" />
        <el-table-column align="left" label="Type" prop="type" width="120" />
        <el-table-column align="left" label="Action">
            <template #default="scope">
            <el-button type="primary" link icon="edit" size="small" class="table-button" @click="updateCampaignLogFunc(scope.row)">Edit</el-button>
...
const getTableData = async() => {
  const table = await getCampaignLogList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteCampaignLogByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
...
// 更新行
const updateCampaignLogFunc = async(row) => {
    const res = await findCampaignLog({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// Delete 行
const deleteCampaignLogFunc = async (row) => {
    const res = await deleteCampaignLog({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: 'successfully deleted'

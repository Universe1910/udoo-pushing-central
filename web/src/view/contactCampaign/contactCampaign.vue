<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="Campaign Id" prop="campaignId" width="120" />
        <el-table-column align="left" label="Action">
            <template #default="scope">
            <el-button type="primary" link icon="edit" size="small" class="table-button" @click="updateContactCampaignFunc(scope.row)">Edit</el-button>
...
const getTableData = async() => {
  const table = await getContactCampaignList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteContactCampaignByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
...
// 更新行
const updateContactCampaignFunc = async(row) => {
    const res = await findContactCampaign({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// Delete 行
const deleteContactCampaignFunc = async (row) => {
    const res = await deleteContactCampaign({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: 'successfully deleted'

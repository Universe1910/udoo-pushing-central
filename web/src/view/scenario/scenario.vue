      ...
      <el-table ref="multipleTable" style="width: 100%" tooltip-effect="dark" :data="tableData" row-key="ID"
        ...
        <el-table-column align="left" label="Action">
          <template #default="scope">
            ...
            <el-button type="primary" link size="small" class="table-button"
              @click="pushToBuilder(scope.row)">Builder</el-button>
            <el-button type="primary" link icon="edit" size="small" class="table-button"
              @click="updateScenarioFunc(scope.row)">Edit</el-button>
...
const rule = reactive({
  name: [{
    required: true,
    message: 'The field can be not empty',
    trigger: ['input', 'blur'],
  }],
  createdBy: [{
    required: true,
...
const getTableData = async () => {
  const table = await getScenarioList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async () => {
  ...
  const res = await deleteScenarioByIds({ ids })
  if (res.code === 0) {
    ElMessage({
      type: 'success',
...
// 更新行
const updateScenarioFunc = async (row) => {
  const res = await findScenario({ ID: row.ID })
  type.value = 'update'
  if (res.code === 0) {
    formData.value = res.data.rescenario
...
// Delete 行
const deleteScenarioFunc = async (row) => {
  const res = await deleteScenario({ ID: row.ID })
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: 'successfully deleted'
...
const enterDialog = async () => {
  elFormRef.value?.validate(async (valid) => {
    ...
    switch (type.value) {
      case 'create':
        res = await createScenario(formData.value)
        break
      case 'update':
        res = await updateScenario(formData.value)
        break
      default:
        res = await createScenario(formData.value)
        break
